...
<section class="flex flex-col gap-4">
	...
	{:else if auth.me?.role === 'ATENDENTE_UBS'}
		<div
			class="border-l-4 border-blue-900 bg-blue-50 px-4 py-2 font-sans text-[12px] text-blue-900"
		>
			Como <strong>Atendente UBS</strong>, você só enxerga relatórios sobre a sua própria produção.
			Dados agregados da unidade ficam com a Coordenação.
		</div>
	{/if}
	...
	<div class="border border-slate-200 bg-white">
		<PanelHeader
			title="1. Escolha o Tipo de Relatório"
	...
	<div class="border border-slate-200 bg-white">
		<PanelHeader
			title="2. Parâmetros da Geração"
			subtitle={catalogoSelecionado
				? `Relatório selecionado: ${catalogoSelecionado.titulo}`
			...
			{#if permiteNominal}
				...
				>
					...
					<span>
						...
						>
						— exigirá justificativa de ≥ 30 caracteres gravada em auditoria. PDF virá com marca d'água
						CONFIDENCIAL.
					</span>
				</label>
			{/if}
