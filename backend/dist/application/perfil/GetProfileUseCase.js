...
const mappers_1 = require("../../infrastructure/database/mappers");
...
class GetProfileUseCase {
    async exec(atendenteId) {
        const a = await prisma_1.prisma.atendente.findUnique({
        ...
        const [hoje, semana, mes, ano, total, aprovados, todosUbs, ultimas, sessaoAtiva, falhasSemana, atividades] = await Promise.all([
            prisma_1.prisma.encaminhamento.count({
            ...
            prisma_1.prisma.encaminhamento.count({ where: { atendenteId } }),
            prisma_1.prisma.encaminhamento.count({
                where: { atendenteId, status: prisma_2.StatusEncaminhamento.APROVADO },
            }),
            prisma_1.prisma.atendente.findMany({
                where: { ubsId: a.ubsId },
                include: { _count: { select: { encaminhamentos: { where: { criadoEm: { gte: inicioMes } } } } } },
            }),
            prisma_1.prisma.encaminhamento.findMany({
                where: { atendenteId },
                select: { criadoEm: true, atualizadoEm: true, especialidadeSolicitada: true },
                take: 200,
                orderBy: { criadoEm: 'desc' },
            }),
            prisma_1.prisma.sessao.findFirst({
                where: { atendenteId, revogadaEm: null, expiraEm: { gt: new Date() } },
                orderBy: { ultimoUsoEm: 'desc' },
            }),
            prisma_1.prisma.tentativaLogin.count({
                where: {
                    atendenteId,
                    sucesso: false,
            ...
            prisma_1.prisma.atividadeAtendente.findMany({
                where: { atendenteId },
                orderBy: { em: 'desc' },
                take: 20,
            }),
        ...
        return {
            ...
            cpf: a.cpf,
            telefone: a.telefone ?? '',
            dataNascimento: (0, mappers_1.ymd)(a.dataNascimento),
            cargo: a.cargo,
            funcao: a.funcao,
            lotacao: `${a.ubs.nome} · ${a.ubs.municipio} / ${a.ubs.uf}`,
            unidade: a.ubs.nome,
            dataAdmissao: (0, mappers_1.ymd)(a.dataAdmissao),
            producao: {
                hoje,
                semana,
                mes,
            ...
            seguranca: {
                senhaAlteradaEm: (0, mappers_1.ymd)(a.senhaAlteradaEm),
